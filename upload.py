...
#BUG --> not finding video in file browser
#BUG --> not pressting enter after finding video,    POSSIBLE SOLUTION: press enter 2x
#POSSIBLE BUG --> when run out of vidoes, dont have time to donwload and edit,   SOLUTION: in crontab call shutdown after hour

# get random time to post (in 45 minutes)
...
# check if have videos, if not, download more videos
def isEmpty(path):
    status = False
    if not os.listdir(path):
         status = True
         print('Directory Empty, Running Download script...')
    else:
         status = False
...
for i in range(1):
    ...
    sleep(1)
    keyboard = Controller()
    keyboard.type(f'/home/server/Downloads/ig-bot/edited/{currentVid}.mp4')
    keyboard.press(Key.enter)
    keyboard.release(Key.enter)
    sleep(2)

    # go thru upload steps
    ...
    # remove
    os.remove(f'edited/{currentVid}.mp4')
    print('----REMOVED----')
